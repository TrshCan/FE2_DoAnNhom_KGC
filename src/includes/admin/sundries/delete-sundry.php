...
$stmt = $conn->prepare("SELECT icon FROM sundries WHERE id = ?");
...
if ($row = $result->fetch_assoc()) {
    ...
    error_log("No sundry found with ID: $id");
}
...
if ($icon) {
    ...
    error_log("No icon associated with sundry ID: $id");
}

// Delete the sundry record from the database
$stmt = $conn->prepare("DELETE FROM sundries WHERE id = ?");
$stmt->bind_param("i", $id);
$stmt->execute();
